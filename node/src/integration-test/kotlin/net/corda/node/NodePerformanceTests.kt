...
class NodePerformanceTests {
    ...
    @Test
    fun `self pay rate`() {
        val user = User("A", "A", setOf(startFlowPermission<CashIssueFlow>(), startFlowPermission<CashPaymentFlow>()))
        driver(
                notarySpecs = listOf(NotarySpec(DUMMY_NOTARY.name, rpcUsers = listOf(user))),
                startNodesInProcess = true,
                extraCordappPackagesToScan = listOf("net.corda.finance")
        ) {
            val notary = notaries[0].nodeHandles.getOrThrow()[0] as NodeHandle.InProcess
            ...
            notary.rpcClientToNode().use("A", "A") { connection ->
                val notaryIdentity = connection.proxy.notaryIdentities()[0]
                println("ISSUING")
                val doneFutures = (1..100).toList().parallelStream().map {
                    connection.proxy.startFlow(::CashIssueFlow, 1.DOLLARS, OpaqueBytes.of(0), notaryIdentity).returnValue
                }.toList()
                ...
                startPublishingFixedRateInjector(metricRegistry, 8, 5.minutes, 100L / TimeUnit.SECONDS) {
                    connection.proxy.startFlow(::CashPaymentFlow, 1.DOLLARS, notary.nodeInfo.chooseIdentity()).returnValue.get()
                }
            }
